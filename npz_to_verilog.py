...
def generate_verilog(global_inputs, gates, conn_a, conn_b, number_of_categories=NUMBER_OF_CATEGORIES, output_bits_per_category=OUTPUT_BITS_PER_CATEGORY):
    ...
    body = ""
    gate_idx = 0
    reg_or_wire = "reg " if PIPELINE_LAYERS else "wire"
    for layer_idx, layer_gates, layer_conn_a, layer_conn_b in zip(range(len(gates)), gates, conn_a, conn_b):
        if layer_idx > 0:
            decl += f"    {reg_or_wire} [{len(gates[layer_idx-1])}:0] layer_{layer_idx-1};\n"
        ...
        if PIPELINE_LAYERS:
            body += f"    always @(posedge clk) begin : pipeline_layer_{layer_idx}\n"
        ...
        for out_idx, gate, a, b in zip(range(len(layer_gates)), layer_gates, layer_conn_a, layer_conn_b):
            ...
            if PIPELINE_LAYERS:
                body += f"        {output}[{out_idx}] <= {op(gate, f'{input_a}', f'{input_b}')};\n"
            else:
    ...
    if number_of_categories > 0:
        ...
        if output_bits_per_category < out_wires_per_category:
            print(f"WARNING! The expected number of output bits ({output_bits_per_category*number_of_categories}) < the size of output layer ({global_outputs}) in the source network.")
            print(f"WARNING! Set OUTPUT_BITS_PER_CATEGORY in `npz_to_verilog.py` and BITS_PER_CATEGORY in `mnist.v` files to {global_outputs//number_of_categories} or higher.")

    verilog = f"""
module net (
    input  wire clk,
    input  wire [{ global_inputs-1}:0] in,
    output {reg_or_wire} [{global_outputs-1}:0] out{"," if number_of_categories > 0 else ""}
