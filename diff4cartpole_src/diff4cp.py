...
GATE_ARCHITECTURE = ast.literal_eval(config.get("GATE_ARCHITECTURE", "[1000,1000,100]")) # previous: [1300,1300,1300] resnet95: [2000, 2000] conn_gain96: [2250, 2250, 2250] power_law_fixed97: [8000,8000,8000, 8000,8000,8000] auto_scale_logits_97_5 [8000]
...
#     # download=True
# )
train_dataset = MyCPSDataset(start=91, end=121, binarize=2, data_dir=train_data_dir, norm=True)
train_dataset.normalize_data()
train_dataset.thermo_encode_data(num_bits_x=num_bits, num_bits_y=num_bits)
...
#     # download=True
# )
test_dataset = MyCPSDataset(start=81, end=86, binarize=2, data_dir=test_data_dir, norm=True)
test_dataset.normalize_data()
test_dataset.thermo_encode_data(num_bits_x=num_bits, num_bits_y=num_bits)

### SPLIT TRAIN DATASET ###

# print(model)
# input = torch.rand((1,224,), dtype=torch.float32, device=device) # dummy input to get the shape
# print(input.shape)
# output = model(input)
# print(output.shape)
...
# test_labels = torch.empty((test_dataset_samples), dtype=torch.long)
    ...
    def validate(dataset="val", model=default_model):
        ...
        for start_idx in range(0, number_of_samples, BATCH_SIZE):
            ...
            with torch.no_grad():
                ...
                val_loss += F.mse_loss(val_output, y_val, reduction="sum").item()
...
for i in range(TRAINING_STEPS):
    ...
    with torch.set_grad_enabled(True):
        ...
        # binarize model_output
        output_bin = model_output.clone()
        ...
        loss_bin = F.mse_loss(torch.sum(output_bin, dim=1)/50 - 1, torch.sum(y_bin, dim=1)/50 - 1)
    ...
    if (i + 1) % PRINTOUT_EVERY == 0:
        ...
        log(f"Iteration {i + 1:10} - Loss {loss_bin:6.3f} - Pass (%) {passthrough_log} | Conn (%) {unique_log} | e-∇x10 {grad_log} | Logits {logits_log}")
        scheduler.step()
...
# # convert loss_record to numpy array and save it
# loss_record = np.array(loss_record, dtype=np.float32)
# loss_record_filename = f"loss_record.npy"
# np.save(loss_record_filename, loss_record)

test_input = torch.rand((1, INPUT_SIZE), dtype=torch.float32, device=device) # dummy input to get the shape
test_input[test_input < 0.5] = 0
test_input[test_input >= 0.5] = 1   
test_output = model(test_input)
print(test_output)
torch.save(model.state_dict(), 'difflg_cartpole_binin_binout.pth')

tloaded_model = Model(SEED, GATE_ARCHITECTURE, INTERCONNECT_ARCHITECTURE, NUMBER_OF_CATEGORIES, INPUT_SIZE).to(device)
tloaded_model.load_state_dict(torch.load('difflg_cartpole_binin_binout.pth'))
test_output_loaded = tloaded_model(test_input)
print(test_output_loaded)
assert torch.equal(test_output, test_output_loaded), "Loaded model output does not match original model output!"
