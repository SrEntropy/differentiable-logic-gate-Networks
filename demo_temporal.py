...
IMG_WIDTH = int(config.get("IMG_WIDTH", 64)) # 16 is suitable for Tiny Tapeout
IMG_CROP = int(config.get("IMG_CROP", 64))
...
GATE_ARCHITECTURE = ast.literal_eval(config.get("GATE_ARCHITECTURE", "[16000,16000,16000]")) # previous: [1300,1300,1300] resnet95: [2000, 2000] conn_gain96: [2250, 2250, 2250] power_law_fixed97: [8000,8000,8000, 8000,8000,8000] auto_scale_logits_97_5 [8000]
INTERCONNECT_ARCHITECTURE = ast.literal_eval(config.get("INTERCONNECT_ARCHITECTURE", "[[],[1],[1]]")) # previous: [[32, 325], [26, 52], [26, 52]])) resnet95, conn_gain96, auto_scale_logits_97_5: [] power_law_fixed97: [[],[-1],[-1], [-1],[-1],[-1]] p
...
EPOCH_STEPS = math.floor((7168 * TRAIN_FRACTION) / BATCH_SIZE) # MNIST consists of 60K images
...
config_printout_keys = ["LOG_NAME", "TIMEZONE",
               "BINARIZE_IMAGE_TRESHOLD", "IMG_WIDTH", "INPUT_SIZE", "MEMORY", "IMG_CROP", "DATA_SPLIT_SEED", "TRAIN_FRACTION", "NUMBER_OF_CATEGORIES", "ONLY_USE_DATA_SUBSET",
...
class Model(nn.Module):
    ...
    def forward(self, X):
        ...
        Xo = torch.zeros(X.shape[0],16000, dtype=torch.float32)
        Xo = Xo.to(device)
        for i in range (8):    #TODO - make intro variable
            Xc = X[:,i,:]
...
log(f"READ DATA")
log(config_printout_dict)
### MOVE TRAIN DATASET TO GPU ###

### PWZ NEW CODE
train = torch.load("demo_train.pt")
