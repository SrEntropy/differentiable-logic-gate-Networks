...
EPOCH_STEPS = math.floor((36212) / BATCH_SIZE) # MNIST consists of 60K images
...
### PWZ NEW CODE
train = torch.load("custom_1ch_train.pt")
...
test = torch.load("custom_1ch_test.pt")
...
# val = torch.load("./datasets/out/custom_test.pt")
# val_images = val["data"]
...
def get_validate(default_model):
    def validate(dataset="val", model=default_model):
        ...
        for start_idx in range(0, number_of_samples, BATCH_SIZE):
            ...
            val_indices = torch.arange(start_idx, end_idx, device="cpu")    
...
with torch.no_grad():
    ...
    include_dataset = {'dataset_input': test_images, 'dataset_output': model_binarized(test_images)}
    print(model_binarized(test_images).shape)
